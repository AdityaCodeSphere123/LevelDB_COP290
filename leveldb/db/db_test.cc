 ...
 public:
  ...
  Status Scan(const ReadOptions& options, const Slice& start_key,
    ...
    for (auto it = map_.lower_bound(start_key.ToString());
         it != map_.end() && options_.comparator->Compare(it->first, end_key) < 0;
         ++it) {
      result->push_back(*it);
    }
    return Status::OK();
...
TEST_F(DBTest, RangeScanEvaluation) {
  // Empty range: Insert a,b,c; scan [x,z)
  ASSERT_LEVELDB_OK(Put("a", "v1"));
  ASSERT_LEVELDB_OK(Put("b", "v2"));
  ASSERT_LEVELDB_OK(Put("c", "v3"));
  std::vector<std::pair<std::string, std::string>> result;
  ASSERT_LEVELDB_OK(db_->Scan(ReadOptions(), "x", "z", &result));
  ASSERT_EQ(0, result.size());

  // Normal scan: Insert a,b,c,d; scan [b,d)
  ASSERT_LEVELDB_OK(Put("d", "v4"));
  result.clear();
  ASSERT_LEVELDB_OK(db_->Scan(ReadOptions(), "b", "d", &result));
  ASSERT_EQ(2, result.size());
  ASSERT_EQ("b", result[0].first);
  ASSERT_EQ("c", result[1].first);

  // Half-open boundary: scan [b,d) includes b, excludes d
  ASSERT_EQ("b", result[0].first);
  ASSERT_EQ("c", result[result.size()-1].first);

  // Updated key: Put b=1, Put b=2, scan [a,c)
  ASSERT_LEVELDB_OK(Put("b", "new_v2"));
  result.clear();
  ASSERT_LEVELDB_OK(db_->Scan(ReadOptions(), "a", "c", &result));
  ASSERT_EQ(2, result.size());
  ASSERT_EQ("a", result[0].first);
  ASSERT_EQ("b", result[1].first);
  ASSERT_EQ("new_v2", result[1].second);

  // Deleted key: Put b, Delete b, scan [a,c)
  ASSERT_LEVELDB_OK(Delete("b"));
  result.clear();
  ASSERT_LEVELDB_OK(db_->Scan(ReadOptions(), "a", "c", &result));
  ...
  // Snapshot scan: Create snapshot before update/delete
  ASSERT_LEVELDB_OK(Put("s", "old"));
  const Snapshot* snap = db_->GetSnapshot();
  ASSERT_LEVELDB_OK(Put("s", "new"));
  result.clear();
  ReadOptions snap_options;
  snap_options.snapshot = snap;
  ASSERT_LEVELDB_OK(db_->Scan(snap_options, "r", "t", &result));
  ASSERT_EQ(1, result.size());
  ASSERT_EQ("s", result[0].first);
  ASSERT_EQ("old", result[0].second);
  db_->ReleaseSnapshot(snap);
}

TEST_F(DBTest, DeleteRangeEvaluation) {
  // Basic range delete: Put a,b,c,d; DeleteRange [b,d)
  ASSERT_LEVELDB_OK(Put("a", "v1"));
  ASSERT_LEVELDB_OK(Put("b", "v2"));
  ASSERT_LEVELDB_OK(Put("c", "v3"));
  ASSERT_LEVELDB_OK(Put("d", "v4"));
  ASSERT_LEVELDB_OK(db_->DeleteRange(WriteOptions(), "b", "d"));
  ASSERT_EQ("v1", Get("a"));
  ASSERT_EQ("NOT_FOUND", Get("b"));
  ASSERT_EQ("NOT_FOUND", Get("c"));
  ASSERT_EQ("v4", Get("d"));

  // Half-open boundary: DeleteRange [b,d) deleted b, but not d
  // (already checked above)

  // Range delete then Put: DeleteRange [b,d), then Put c=new
  ASSERT_LEVELDB_OK(Put("c", "new_v3"));
  ASSERT_EQ("new_v3", Get("c"));

  // Put then Range delete: Put e=old, DeleteRange [d,f)
  ASSERT_LEVELDB_OK(Put("e", "old_v5"));
  ASSERT_LEVELDB_OK(db_->DeleteRange(WriteOptions(), "d", "f"));
  ASSERT_EQ("NOT_FOUND", Get("d"));
  ASSERT_EQ("NOT_FOUND", Get("e"));

  // Interaction with point delete: Delete c and DeleteRange overlap
  ASSERT_LEVELDB_OK(Put("g", "v7"));
  ASSERT_LEVELDB_OK(Delete("g"));
  ASSERT_LEVELDB_OK(db_->DeleteRange(WriteOptions(), "f", "h"));
  ASSERT_EQ("NOT_FOUND", Get("g"));

  // After compaction: DeleteRange, ForceFullCompaction, scan/get
  ASSERT_LEVELDB_OK(Put("h", "v8"));
  ASSERT_LEVELDB_OK(Put("i", "v9"));
  ASSERT_LEVELDB_OK(db_->DeleteRange(WriteOptions(), "h", "j"));
  ASSERT_LEVELDB_OK(db_->ForceFullCompaction());
  ASSERT_EQ("NOT_FOUND", Get("h"));
  ASSERT_EQ("NOT_FOUND", Get("i"));
}

TEST_F(DBTest, ForceFullCompactionEvaluation) {
  // Basic full compaction: Insert enough keys to create multiple SSTables
  for (int i = 0; i < 1000; i++) {
    char buf[20];
    snprintf(buf, sizeof(buf), "key%04d", i);
    ASSERT_LEVELDB_OK(Put(buf, "value"));
  }
  // Flush to L0
  dbfull()->TEST_CompactMemTable();
  // Call ForceFullCompaction
  ASSERT_LEVELDB_OK(db_->ForceFullCompaction());

  // Deleted key cleanup
  ASSERT_LEVELDB_OK(Put("compaction_test_key", "v1"));
  ASSERT_LEVELDB_OK(Delete("compaction_test_key"));
  ASSERT_LEVELDB_OK(db_->ForceFullCompaction());
  ASSERT_EQ("NOT_FOUND", Get("compaction_test_key"));

  // Range tombstone cleanup
  for (int i = 100; i < 200; i++) {
    char buf[20];
    snprintf(buf, sizeof(buf), "k%03d", i);
    ASSERT_LEVELDB_OK(Put(buf, "v"));
  }
  ASSERT_LEVELDB_OK(db_->DeleteRange(WriteOptions(), "k100", "k200"));
  ASSERT_LEVELDB_OK(db_->ForceFullCompaction());
  for (int i = 100; i < 200; i++) {
    char buf[20];
    snprintf(buf, sizeof(buf), "k%03d", i);
    ASSERT_EQ("NOT_FOUND", Get(buf));
  }
