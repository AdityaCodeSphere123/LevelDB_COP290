...
TEST_F(DBTest, ForceFullCompactionIsolation) {
  ...
  // ADD THIS SECOND WAVE: Force an overlap so data stays in L0!
  for (int i = 0; i < 200; i++) {
    ASSERT_LEVELDB_OK(Put("key" + std::to_string(i), std::string(1000, 'y')));
  }
  dbfull()->TEST_CompactMemTable();
...
TEST_F(FullCompactionTest, T15_SnapshotPreservedAcrossFFC) {
  ...
  ASSERT_LEVELDB_OK(db_->Put(wo, "snap_key", "new_val"));
  ASSERT_LEVELDB_OK(FillRandom(300));
  ASSERT_LEVELDB_OK(db_->ForceFullCompaction());
  ...
  ro.snapshot = snap;
  std::string val;
  ASSERT_LEVELDB_OK(db_->Get(ro, "snap_key", &val));
